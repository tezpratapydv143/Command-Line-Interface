...
protected void commandControl (KeyEvent ke) {
		...
		if (key==KeyEvent. VK_DOWN) {
			...
			if (c=='\n') {
				...
				tempCommand=tempCommand+1;
				//System.out.println(11+" "+commandNum);
...
for (j=0;j<=fileNames.length;j++) {
//File f=new File(drive_Name+currentPath+fileNames[j]); //System.out.println(currentPath);
...
if (token.equals("-fr")) { ta.append("France: \n");
country=Locale. FRANCE;
}
if (token.equals("-gr")) { ta.append("Germany: \n");
...
ta.append(" kill    | Deletes file                | kill filel.txt file2.txt\n");
ta.append(" save    | Saves file in edit mode     |save filel file2 file3\n");
...
myFrame.show();
//myFrame.setVisible(true);  // Use setVisible(true) instead of show()
